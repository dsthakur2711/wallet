...
func (u *userService) CreateUser(ctx context.Context, createUserDto dto.CreateUserDto) (dto.UserDto, error) {
	...
	_, err = u.userRepo.GetUserByUsername(ctx, createUserDto.Username)
	if err == local_errors.ErrSomethingWrong{
		return userDto, err
	}
	if err == nil{
		//this means username already exist
		return userDto, local_errors.ErrUsernameAlreadyTaken
	}
